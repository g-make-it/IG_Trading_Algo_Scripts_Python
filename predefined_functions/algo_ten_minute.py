...
class Algo0:
    def __init__(self):
        ...
        self.map_epic_data_10_minute={}
        for epic in list_of_epics:
            self.map_epic_data_10_minute[epic] = []
    ...
    def run(self):
        ...
        while(True):
            try:
                ...
                for epic in self.map_epic_data_10_minute.keys():
    ...
    def create_orders(self, epic, signals_levels):
        ...
        order = self.df.create_working_order_at_point(epic=epic, direction=key,price_order_level=signals_levels[key])
        return order
    ...
    def signal_generation(self, epic):
        ...
        if (self.first_timestamp != None):
            ...
            if (difference.seconds > minute_10):
                ...
                self.map_epic_data_10_minute[epic].append(data)
        ...
        if len(self.map_epic_data_10_minute[epic]) > 4:
            self.map_epic_data_10_minute[epic].pop(0)
            ...
            object_epic_data = self.map_epic_data_10_minute[epic][0]
            ...
            for i in range(1, len(self.map_epic_data_10_minute[epic])):
                
                object_epic_data = self.map_epic_data_10_minute[epic][i]
                bid = object_epic_data["snapshot"]["bid"] - bid
                offer = object_epic_data["snapshot"]["offer"] - offer
                high = object_epic_data["snapshot"]["high"] - high
                low = object_epic_data["snapshot"]["low"] - low
                
                difference_bid.append(bid)
                difference_offer.append(offer)
                difference_low.append(low)
                difference_high.append(high)
                # the high/low becomes the levels we enter orders at
                bid = object_epic_data["snapshot"]["bid"]
                offer = object_epic_data["snapshot"]["offer"]
                high = object_epic_data["snapshot"]["high"]
                low = object_epic_data["snapshot"]["low"]
            ...
            offer_average = sum(difference_offer)/len(difference_offer)
            bid_average = sum(difference_bid)/len(difference_bid)

            # we have gone long we should now short
            if (bid_average > 0 and offer_average> 0):
                sell_level = high
            elif (bid_average < 0 and offer_average < 0):
                buy_level = low
    ...
    def finding_lows_highs(self, data, reset = False):
        ...
        if reset:
            ...
            object_dict = self.map_epic_data_10_minute[epic][-1]
            object_dict["snapshot"]["high"] = self.high
            object_dict["snapshot"]["low"] = self.low
            self.map_epic_data_10_minute[epic][-1] = object_dict
